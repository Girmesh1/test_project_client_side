...
import { addSong, deleteSong, getSongsSuccess } from '../src/store/slice/songslice';
...
function* updateSongOnServer(action) {
    ...
    const songs = yield call (() => axios.patch (`http://localhost:8000/songs/${action.payload.id}`,{title,album,artist,genre}));
...
function* deleteSongFromServer(action) {
  ...
  try {
    yield call(axios.delete, `http://localhost:8000/songs/${action.payload.id}`);
...
const MySaga = { songSaga, addSongSaga, updateSongSaga, deleteSongSaga };
